...
`test/functional/test_framework/test_framework.py:HyliumTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'feature_hylium_security_check.py',
    'feature_hylium_wallet_basic.py',
    'feature_hylium_mining_basic.py',
    'feature_taproot.py',
    'feature_block.py',
    ...
    'interface_hylium_cli.py',
    'feature_bind_extra.py',
    'mempool_resurrect.py',
    'wallet_txn_doublespend.py --mineblock',
    'tool_hylium_chainstate.py',
    'tool_wallet.py',
    'tool_utils.py',
...
def main():
    ...
    enable_hyliumd = config["components"].getboolean("ENABLE_HYLIUMD")

    if not enable_hyliumd:
...
def run_tests(*, test_list, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, results_filepath=None):
    ...
    # Warn if hyliumd is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "hyliumd"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a hyliumd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
    ...
    if shutil.disk_usage(tmpdir).free < min_space:
        print(f"{BOLD[1]}WARNING!{BOLD[0]} There may be insufficient free space in {tmpdir} to run the Hylium functional test suite. "
    ...
    # a hard link or a copy on any platform. See https://github.com/hylium/hylium/pull/27561.
...
class RPCCoverage():
    ...
    commands per `hylium-cli help` (`rpc_interface.txt`).
