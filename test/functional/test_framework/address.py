...
"""Encode and decode Hylium addresses.
...
ADDRESS_BCRT1_UNSPENDABLE = 'rhyl1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqa3ad9z'
ADDRESS_BCRT1_UNSPENDABLE_DESCRIPTOR = 'addr(rhyl1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqa3ad9z)#sjk8zqcm'
# Coins sent to this address can be spent with a witness stack of just OP_TRUE
ADDRESS_BCRT1_P2WSH_OP_TRUE = 'rhyl1qft5p2uhsdcdc3l2ua4ap5qqfg4pjaqlp250x7us7a8qqhrxrxfsquwuvxy'
...
def create_deterministic_address_bcrt1_p2tr_op_true(explicit_internal_key=None):
    ...
    if explicit_internal_key is None:
        assert_equal(address, 'rhyl1p9yfmy5h72durp7zrhlw9lf7jpwjgvwdg0jr0lqmmjtgg83266lqs4pu5hd')
...
def keyhash_to_p2pkh(hash, main=False):
    assert len(hash) == 20
    version = 38 if main else 111
    return byte_to_base58(hash, version)

def scripthash_to_p2sh(hash, main=False):
    assert len(hash) == 20
    version = 53 if main else 196
    return byte_to_base58(hash, version)
...
def program_to_witness(version, program, main=False):
    ...
    return encode_segwit_address("hyl" if main else "rhyl", version, program)
