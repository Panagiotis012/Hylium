#!/usr/bin/env python3
# Copyright (c) 2014-present The Hylium Core developers
...
TMPDIR_PREFIX = "hylium_func_test_"
...
class HyliumTestMetaClass(type):
    """Metaclass for HyliumTestFramework.

    Ensures that any attempt to register a subclass of `HyliumTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'HyliumTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("HyliumTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("HyliumTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class HyliumTestFramework(metaclass=HyliumTestMetaClass):
    """Base class for a hylium test script.

    Individual hylium test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave hyliumds and test.* datadir on exit or error")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use hylium-cli instead of RPC for all commands")
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("HyliumRPC")
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a hyliumd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple hyliumd test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the hyliumd
    ...
    def _start_logging(self):
        ...
        # Format logs the same as hyliumd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in hylium.conf
    ...
    def skip_if_no_hyliumd_tracepoints(self):
        """Skip the running test if hyliumd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("hyliumd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_hyliumd_zmq(self):
        """Skip the running test if hyliumd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("hyliumd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if hylium-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("hylium-wallet has not been compiled")

    def skip_if_no_hylium_tx(self):
        """Skip the running test if hylium-tx has not been compiled."""
        if not self.is_hylium_tx_compiled():
            raise SkipTest("hylium-tx has not been compiled")

    def skip_if_no_hylium_util(self):
        """Skip the running test if hylium-util has not been compiled."""
        if not self.is_hylium_util_compiled():
            raise SkipTest("hylium-util has not been compiled")

    def skip_if_no_hylium_chainstate(self):
        """Skip the running test if hylium-chainstate has not been compiled."""
        if not self.is_hylium_chainstate_compiled():
            raise SkipTest("hylium-chainstate has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if hylium-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("hylium-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether hylium-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether hylium-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_hylium_tx_compiled(self):
        """Checks whether hylium-tx was compiled."""
        return self.config["components"].getboolean("BUILD_HYLIUM_TX")

    def is_hylium_util_compiled(self):
        """Checks whether hylium-util was compiled."""
        return self.config["components"].getboolean("ENABLE_HYLIUM_UTIL")

    def is_hylium_chainstate_compiled(self):
        """Checks whether hylium-chainstate was compiled."""
        return self.config["components"].getboolean("ENABLE_HYLIUM_CHAINSTATE")
