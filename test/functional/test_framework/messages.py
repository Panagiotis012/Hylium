...
# Copyright (c) 2010-2022 The Hylium Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""Hylium test framework primitive and message structures
...
COIN = 100000000  # 1 hyl in satoshis
...
MAGIC_BYTES = {
    "mainnet": b"\xf3\xc2\xe1\xb0",
    "testnet4": b"\xb3\xa1\x7f\x55",
    "regtest": b"\xc1\xd2\xe3\xf4",
...
# Objects that map to hyliumd objects, which can be serialized/deserialized


class CAddress:
    __slots__ = ("net", "ip", "nServices", "port", "time")

    # see https://github.com/hylium/bips/blob/master/bip-0155.mediawiki
    NET_IPV4 = 1
    NET_IPV6 = 2
    NET_TORV3 = 4
    NET_I2P = 5
...
class CBlockLocator:
    ...
    def serialize(self):
        r = b""
        r += (0).to_bytes(4, "little", signed=True)  # Hylium Core ignores the version field. Set it to 0.
...
class CTransaction:
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in hyliumd
            if (flags != 0):
                self.vin = deser_vector(f, CTxIn)
...
class msg_version:
    ...
    def deserialize(self, f):
        ...
        # But, unconditionally check it to match behaviour in hyliumd
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in hyliumd indicates these should be deserialized as blocks
