...
def assert_fee_amount(fee, tx_size, feerate_HYL_kvB):
    """Assert the fee is in range."""
    assert isinstance(tx_size, int)
    target_fee = get_fee(tx_size, feerate_HYL_kvB)
    if fee < target_fee:
        raise AssertionError("Fee of %s HYL too low! (Should be %s HYL)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    high_fee = get_fee(tx_size + 2, feerate_HYL_kvB)
    if fee > high_fee:
        raise AssertionError("Fee of %s HYL too high! (Should be %s HYL)" % (str(fee), str(target_fee)))
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting HYL values"""
...
class Binaries:
    """Helper class to provide information about hylium binaries
    ...
    def node_argv(self, **kwargs):
        "Return argv array that should be used to invoke hyliumd"
        return self._argv("node", self.paths.hyliumd, **kwargs)

    def rpc_argv(self):
        "Return argv array that should be used to invoke hylium-cli"
        # Add -nonamed because "hylium rpc" enables -named by default, but hylium-cli doesn't
        return self._argv("rpc", self.paths.hyliumcli) + ["-nonamed"]

    def tx_argv(self):
        "Return argv array that should be used to invoke hylium-tx"
        return self._argv("tx", self.paths.hyliumtx)

    def util_argv(self):
        "Return argv array that should be used to invoke hylium-util"
        return self._argv("util", self.paths.hyliumutil)

    def wallet_argv(self):
        "Return argv array that should be used to invoke hylium-wallet"
        return self._argv("wallet", self.paths.hyliumwallet)

    def chainstate_argv(self):
        "Return argv array that should be used to invoke hylium-chainstate"
        return self._argv("chainstate", self.paths.hyliumchainstate)

    def _argv(self, command, bin_path, need_ipc=False):
        """Return argv array that should be used to invoke the command. It
        either uses the hylium wrapper executable (if HYLIUM_CMD is set or
        need_ipc is True), or the direct binary path (hyliumd, etc). When
        bin_dir is set (by tests calling binaries from previous releases) it
        always uses the direct path."""
        if self.bin_dir is not None:
            return [os.path.join(self.bin_dir, os.path.basename(bin_path))]
        elif self.paths.hylium_cmd is not None or need_ipc:
            # If the current test needs IPC functionality, use the hylium
            # wrapper binary and append -m so it calls multiprocess binaries.
            hylium_cmd = self.paths.hylium_cmd or [self.paths.hylium_bin]
            return hylium_cmd + (["-m"] if need_ipc else []) + [command]
        else:
            return [bin_path]
...
def get_binary_paths(config):
    ...
    binaries = {
        "hylium": "HYLIUM_BIN",
        "hyliumd": "HYLIUMD",
        "hylium-cli": "HYLIUMCLI",
        "hylium-util": "HYLIUMUTIL",
        "hylium-tx": "HYLIUMTX",
        "hylium-chainstate": "HYLIUMCHAINSTATE",
        "hylium-wallet": "HYLIUMWALLET",
    }
    # Set paths to hylium core binaries allowing overrides with environment
    # variables.
    ...
    # HYLIUM_CMD environment variable can be specified to invoke hylium
    # wrapper binary instead of other executables.
    paths.hylium_cmd = shlex.split(os.getenv("HYLIUM_CMD", "")) or None
    return paths
...
def get_fee(tx_size, feerate_hyl_kvb):
    """Calculate the fee in HYL given a feerate is HYL/kvB. Reflects CFeeRate::GetFee"""
    feerate_sat_kvb = int(feerate_hyl_kvb * Decimal(1e8)) # Fee in sat/kvb as an int to avoid float precision errors
    target_fee_sat = ceildiv(feerate_sat_kvb * tx_size, 1000) # Round calculated fee up to nearest sat
    return target_fee_sat / Decimal(1e8) # Return result in  HYL
...
def wait_until_helper_internal(predicate, *, timeout=60, lock=None, timeout_factor=1.0, check_interval=0.05):
    ...
    from `HyliumTestFramework` or `P2PInterface` class ensures the timeout is
...
def bpf_cflags():
    return [
        ...
        "-Wno-duplicate-decl-specifier",  # https://github.com/hylium/hylium/issues/32322
    ]
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "hylium.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> tuple[dict, pathlib.Path]:
    ...
    if platform.system() == "Windows":
        ...
        datadir = temp_dir / "Hylium"
    else:
        env = dict(HOME=str(temp_dir))
        if platform.system() == "Darwin":
            datadir = temp_dir / "Library/Application Support/Hylium"
        else:
            datadir = temp_dir / ".hylium"
    return env, datadir
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "hylium.conf"), 'a') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "hylium.conf")):
        with open(os.path.join(datadir, "hylium.conf"), 'r') as f:
            for line in f:
