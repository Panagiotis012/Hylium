#!/usr/bin/env python3
# Copyright (c) The Hylium Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""Test the hylium wrapper tool."""
...
class ToolHyliumTest(HyliumTestFramework):
    def set_test_params(self):
        self.setup_clean_chain = True
        self.num_nodes = 1

    def skip_test_if_missing_module(self):
        # Skip test on windows because currently when `hylium node -version` is
        # run on windows, python doesn't capture output from the child
        # `hyliumd` and `hylium-node` process started with _wexecvp, and
        # stdout/stderr are always empty. See
        # https://github.com/hylium/hylium/pull/33229#issuecomment-3265524908
    ...
    def set_cmd_args(self, node, args):
        """Set up node so it will be started through hylium wrapper command with specified arguments."""
        node.args = [self.binary_paths.hylium_bin] + args + ["node"] + self.node_options[node.index]
    ...
    def run_test(self):
        ...
        self.log.info("Ensure hylium node command invokes hyliumd by default")
        self.test_args([], [], expect_exe="hyliumd")

        self.log.info("Ensure hylium -M invokes hyliumd")
        self.test_args(["-M"], [], expect_exe="hyliumd")

        self.log.info("Ensure hylium -M does not accept -ipcbind")
        ...
        if self.is_ipc_compiled():
            self.log.info("Ensure hylium -m invokes hylium-node")
            self.test_args(["-m"], [], expect_exe="hylium-node")

            self.log.info("Ensure hylium -m does accept -ipcbind")
            self.test_args(["-m"], ["-ipcbind=unix"], expect_exe="hylium-node")

            self.log.info("Ensure hylium accepts -ipcbind by default")
            self.test_args([], ["-ipcbind=unix"], expect_exe="hylium-node")

            self.log.info("Ensure hylium recognizes -ipcbind in config file")
            append_config(node.datadir_path, ["ipcbind=unix"])
            self.test_args([], [], expect_exe="hylium-node")
