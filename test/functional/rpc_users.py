...
class HTTPBasicsTest(HyliumTestFramework):
    def set_test_params(self):
        self.num_nodes = 2
        self.supports_cli = False

    def conf_setup(self):
        #Append rpcauth to hylium.conf before initialization
        ...
        with open(self.nodes[0].datadir_path / "hylium.conf", "a") as f:
            f.write(rpcauth + "\n")
            f.write(rpcauth2 + "\n")
            f.write(rpcauth3 + "\n")
        with open(self.nodes[1].datadir_path / "hylium.conf", "a") as f:
            f.write("rpcuser={}\n".format(self.rpcuser))
    ...
    def run_test(self):
        ...
        assert_equal(401, call_with_auth(self.nodes[0], 'user1', 'hylium').status)
