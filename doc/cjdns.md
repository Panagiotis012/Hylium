# CJDNS support in Hylium Core

It is possible to run Hylium Core over CJDNS, an encrypted IPv6 network that
...
done outside of Hylium Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Hylium network and individual nodes.
...
before it will work with your Hylium Core node. This is described in steps
...
## Run Hylium Core with CJDNS

Once you are connected to the CJDNS network, the following Hylium Core
...
When enabled, this option tells Hylium Core that it is running in an
...
IPv6 local network. This helps Hylium Core perform better address management:
...
CJDNS support was added to Hylium Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `hylium-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/hylium/hylium/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Hylium Core

There are several ways to see your CJDNS address in Hylium Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `hylium-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `hylium-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `hylium-cli help getnodeaddresses` for details.
