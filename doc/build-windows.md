...
Below are some notes on how to build Hylium Core for Windows.

The options known to work for building Hylium Core on Windows are:
...
Note that for WSL the Hylium Core source path MUST be somewhere in the default mount file system, for
example /usr/src/hylium, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\hylium`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/hylium
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/hylium --strip
```
