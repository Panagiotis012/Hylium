...
Hylium Core version 0.20.0 is now available from:

  <https://hyliumcore.org/bin/hylium-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Hylium Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Hylium Core version 0.18.0.
...
- Testing or debugging of implementations of the Hylium P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Hylium Core. Hylium Core logs debug messages
...
- OpenSSL is no longer used by Hylium Core.  (#17265)

- BIP70 support has been fully removed from Hylium Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/hylium/hylium/issues/16599),
  [PR #16702](https://github.com/hylium/hylium/pull/16702), and the
  `hyliumd help` for more information.  This option is experimental and
...
- All custom settings configured when Hylium Core starts are now
...
- The "Start Hylium Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://hyliumcore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Hylium Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Hylium Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/hylium-core/HWI) for
  signing.  Future versions of Hylium Core should support a GUI option
...
- Hylium Core's automatically-generated source code documentation is
  now available at https://doxygen.hyliumcore.org.  (#17596)
...
- The `hylium-cli` utility used with the `-getinfo` parameter now
...
- The `hylium-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `hyliumstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_hylium-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBHYLIUM_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_hylium` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBHYLIUM_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for hylium-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate hylium schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest hylium.conf (sdaftuar)
...
- #18022 Fix appveyor `test_hylium` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (hylium-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone hylium-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Hylium Core files (hebasto)
...
- #17119 Fix broken hylium-cli examples (andrewtoth)
...
- #17503 Remove bitness from hylium-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.hyliumcore.org (laanwj)
...
- #17648 Rename wallet-tool references to hylium-wallet (hel-o)
...
- #18341 Replace remaining literal HYL with `CURRENCY_UNIT` (domob1812)
...
- #16115 On hyliumd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from hylium-cli -getinfo (malevolent)
