...
Hylium Core version v30.0 is now available from:

  <https://hyliumcore.org/bin/hylium-core-30.0/>
...
installer (on Windows) or just copy over `/Applications/Hylium-Qt` (on macOS)
...
Hylium Core is supported and tested on operating systems using the
Linux Kernel 3.17+, macOS 13+, and Windows 10+. Hylium
...
- A new `hylium` command line tool has been added to make features more discoverable
  and convenient to use. The `hylium` tool just calls other executables and does not
  implement any functionality on its own. Specifically `hylium node` is a synonym for
  `hyliumd`, `hylium gui` is a synonym for `hylium-qt`, and `hylium rpc` is a synonym
  for `hylium-cli -named`. Other commands and options can be listed with `hylium help`.
  The new `hylium` command is an alternative to calling other commands directly, but it
...
- The new `hylium` command does support one new feature: an (experimental) IPC Mining
  Interface that allows the node to work with Stratum v2 or other mining client software,
  see (#31098). When the node is started with `hylium -m node -ipcbind=unix` it will
  ...
  (`hylium-node` instead of `hyliumd`) and is currently required but will become optional
  in the future (with [#33229](https://github.com/hylium/hylium/pull/33229)).

- IPC connectivity introduces new dependencies (see [multiprocess.md](https://github.com/hylium/hylium/blob/master/doc/multiprocess.md)),
...
- The `test_hylium` executable is now installed in `libexec/` instead of `bin/`.
  It can still be executed directly, or accessed through the new `hylium` command
  as `hylium test`. The `libexec/` directory also contains new `hylium-node` and
  `hylium-gui` binaries which support IPC features and are called through the
  `hylium` tool. In source builds only, `test_hylium-qt`, `bench_hylium`, and
  `hylium-chainstate` are also now installed to `libexec/` instead of `bin/` and
  can be accessed through the new `hylium` command. See `hylium help` output for
  details. (#31679)
...
- The implementation of coinstatsindex was changed to prevent an overflow bug that
  ...
  from their datadir. A future release of Hylium Core may remove the old version of the
...
- The `-paytxfee` startup option and the `settxfee` RPC are now deprecated and
  will be removed in Hylium Core 31.0. They allowed the user to set a static fee
...
- The legacy wallet removal drops redundant options in the hylium-wallet tool,
...
[Transifex](https://explore.transifex.com/hylium/hylium/).
