# Fuzzing Hylium Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Hylium Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/hylium/hylium
$ cd hylium/
...
## Overview of Hylium Core fuzzing

[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/hylium_report.pdf) includes an external overview of the status of Hylium Core fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.

## Fuzzing harnesses and output

[`process_message`](https://github.com/hylium/hylium/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/hylium/hylium/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/hylium/hylium/tree/master/src/test/fuzz).
...
It is possible to specify `hyliumd` arguments to the `fuzz` executable.
...
The project's collection of seed corpora is found in the [`hylium-core/qa-assets`](https://github.com/hylium-core/qa-assets) repo.

To fuzz `process_message` using the [`hylium-core/qa-assets`](https://github.com/hylium-core/qa-assets) seed corpus:

```sh
$ git clone --depth=1 https://github.com/hylium-core/qa-assets
...
an instrumentalized libc++, then using it to build [Hylium Core dependencies
from source](../depends/README.md) and finally the Hylium Core fuzz binary
...
the qa-assets repo
(https://github.com/hylium-core/qa-assets/blob/main/.github/PULL_REQUEST_TEMPLATE.md).
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`hylium-core/qa-assets`](https://github.com/hylium-core/qa-assets) repo.

Every single pull request submitted against the Hylium Core repo is automatically tested against all inputs in the [`hylium-core/qa-assets`](https://github.com/hylium-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Hylium Core more robust.
...
# Fuzzing Hylium Core using afl++

## Quickstart guide

To quickly get started fuzzing Hylium Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Hylium Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Hylium Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
Hylium Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/hylium-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://issues.oss-fuzz.com/issues?q=hylium-core%20status:open).

Hylium Core follows its [security disclosure policy](https://hyliumcore.org/en/security-advisories/),
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_hylium-core/latest).
