...
#include <hylium-build-config.h> // IWYU pragma: keep

#include <qt/hyliumgui.h>
...
const std::string HyliumGUI::DEFAULT_UIPLATFORM =
...
HyliumGUI::HyliumGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &HyliumGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &HyliumGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &HyliumGUI::showModalOverlay);
...
void HyliumGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Hylium address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and hylium: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &HyliumGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &HyliumGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &HyliumGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Hylium addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Hylium addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Hylium Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Hylium Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a hylium: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Hylium command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &HyliumGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &HyliumGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &HyliumGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &HyliumGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &HyliumGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &HyliumGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &HyliumGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &HyliumGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &HyliumGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &HyliumGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &HyliumGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &HyliumGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &HyliumGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &HyliumGUI::showDebugWindow);
}

void HyliumGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void HyliumGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &HyliumGUI::setCurrentWalletBySelectorIndex);
...
void HyliumGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &HyliumGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &HyliumGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &HyliumGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &HyliumGUI::showProgress);
...
#ifdef ENABLE_WALLET
void HyliumGUI::enableHistoryAction(bool privacy)
{
...
void HyliumGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &HyliumGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &HyliumGUI::removeWallet);
...
WalletController* HyliumGUI::getWalletController()
{
    return m_wallet_controller;
}

void HyliumGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &HyliumGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &HyliumGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &HyliumGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &HyliumGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &HyliumGUI::incomingTransaction);
    connect(this, &HyliumGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void HyliumGUI::removeWallet(WalletModel* walletModel)
{
...
void HyliumGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void HyliumGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void HyliumGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void HyliumGUI::setWalletActionsEnabled(bool enabled)
{
...
void HyliumGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &HyliumGUI::toggleHidden);
...
void HyliumGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void HyliumGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void HyliumGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void HyliumGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void HyliumGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void HyliumGUI::openClicked()
{
...
void HyliumGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void HyliumGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void HyliumGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void HyliumGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void HyliumGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void HyliumGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void HyliumGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void HyliumGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Hylium network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void HyliumGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void HyliumGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void HyliumGUI::updateHeadersSyncProgressLabel()
{
...
void HyliumGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void HyliumGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &HyliumGUI::quitRequested);
...
void HyliumGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void HyliumGUI::createWallet()
{
#ifdef ENABLE_WALLET
    auto activity = new CreateWalletActivity(getWalletController(), this);
    connect(activity, &CreateWalletActivity::created, this, &HyliumGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void HyliumGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void HyliumGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &HyliumGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &HyliumGUI::show);
                e->ignore();
            }
        }
    }
...
void HyliumGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void HyliumGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void HyliumGUI::incomingTransaction(const QString& date, HyliumUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(HyliumUnits::formatWithUnit(unit, amount, true));
...
void HyliumGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void HyliumGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool HyliumGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool HyliumGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void HyliumGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void HyliumGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void HyliumGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void HyliumGUI::updateProxyIcon()
{
    std::string ip_port;
...
void HyliumGUI::updateWindowTitle()
{
...
void HyliumGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void HyliumGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void HyliumGUI::detectShutdown()
{
...
void HyliumGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void HyliumGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(HyliumGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = HyliumGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void HyliumGUI::subscribeToCoreSignals()
{
...
void HyliumGUI::unsubscribeFromCoreSignals()
{
...
bool HyliumGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<HyliumUnit> units = HyliumUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const HyliumUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, HyliumUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const HyliumUnit u : HyliumUnits::availableUnits()) {
        menu->addAction(HyliumUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(HyliumUnit newUnits)
{
    setText(HyliumUnits::longName(newUnits));
}
