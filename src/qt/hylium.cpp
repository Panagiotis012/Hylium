...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<HyliumUnit>("HyliumUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. hylium_de.qm (shortcut "de" needs to be defined in hylium.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. hylium_de_DE.qm (shortcut "de_DE" needs to be defined in hylium.qrc)
...
static const char* qt_argv = "hylium-qt";

HyliumApplication::HyliumApplication()
...
void HyliumApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the HyliumApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", HyliumGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void HyliumApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool HyliumApplication::createOptionsModel(bool resetSettings)
{
...
void HyliumApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new HyliumGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &HyliumGUI::quitRequested, this, &HyliumApplication::requestShutdown);
...
void HyliumApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void HyliumApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool HyliumApplication::baseInitialize()
{
    return node().baseInitialize();
}

void HyliumApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &HyliumApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &HyliumApplication::handleRunawayException);
    connect(this, &HyliumApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &HyliumApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void HyliumApplication::parameterSetup()
{
...
void HyliumApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void HyliumApplication::requestInitialize()
{
...
void HyliumApplication::requestShutdown()
{
...
void HyliumApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // hylium: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &HyliumGUI::handlePaymentRequest);
            connect(window, &HyliumGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void HyliumApplication::handleRunawayException(const QString &message)
{
...
void HyliumApplication::handleNonFatalException(const QString& message)
{
...
WId HyliumApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool HyliumApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", HyliumGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(hylium);
    Q_INIT_RESOURCE(hylium_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(HYLIUM_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see hylium-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see hylium-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse hylium.conf, determine network, switch to network specific
    ...
    // hylium: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
