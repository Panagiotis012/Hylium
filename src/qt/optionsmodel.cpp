...
/** Call node.updateRwSetting() with Hylium 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Hylium 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/hylium/hylium/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because hylium 22.x will not try to read these.
...
//! Convert enabled/size values to hylium -prune setting.
...
//! Get pruning enabled value to show in GUI from hylium -prune setting.
...
//! Get pruning size value to show in GUI from hylium -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayHyliumUnit")) {
        settings.setValue("DisplayHyliumUnit", QVariant::fromValue(HyliumUnit::HYL));
    }
    QVariant unit = settings.value("DisplayHyliumUnit");
    if (unit.canConvert<HyliumUnit>()) {
        m_display_hylium_unit = unit.value<HyliumUnit>();
    } else {
        m_display_hylium_unit = HyliumUnit::HYL;
        settings.setValue("DisplayHyliumUnit", QVariant::fromValue(m_display_hylium_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if hylium.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_hylium_unit);
...
{
    if (new_unit.isNull() || new_unit.value<HyliumUnit>() == m_display_hylium_unit) return;
    m_display_hylium_unit = new_unit.value<HyliumUnit>();
    QSettings settings;
    settings.setValue("DisplayHyliumUnit", QVariant::fromValue(m_display_hylium_unit));
    Q_EMIT displayUnitChanged(m_display_hylium_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/hylium/hylium/pull/8273
    ...
    // (https://github.com/hylium-core/gui/issues/567).
