...
const char * const HYLIUM_CONF_FILENAME = "hylium.conf";
const char * const HYLIUM_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //hylium-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", HYLIUM_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Hylium
    //   new: C:\Users\Username\AppData\Local\Hylium
    // macOS: ~/Library/Application Support/Hylium
    // Unix-like: ~/.hylium
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Hylium";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Hylium";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Hylium";
#else
    // Unix-like
    return pathRet / ".hylium";
#endif
#endif
}
