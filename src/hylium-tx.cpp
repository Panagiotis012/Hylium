...
static void SetupHyliumTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " hylium-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The hylium-tx tool is used for creating and modifying hylium transactions.\n\n"
                "hylium-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded hylium transaction, or with \"-create [commands]\" to create a hex-encoded hylium transaction.\n"
                "\n"
                "Usage: hylium-tx [options] <hex-tx> [commands]\n"
                "or:    hylium-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded hylium transaction
