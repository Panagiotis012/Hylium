...
//! Specify whether parameter should be parsed by hylium-cli as a JSON value,
 ...
 *
 * JSON parameters need to be listed here to make hylium-cli parse command line
 * arguments as JSON, instead of passing them as raw strings. `JSON` and
 * `JSON_OR_STRING` formats both make `hylium-cli` attempt to parse the
 ...
 * `hylium-cli getblockstats <hash>` without needing to quote the hash string
 * as JSON (`'"<hash>"'`).
 *
 ...
 * to make hylium-cli treat them as positional parameters when `-named` is used.
 * This prevents `hylium-cli` from splitting strings like "my=wallet" into a named
 ...
 * other string parameters for that method need to be listed as well so hylium-cli
 ...
 *
 * For example, the command `hylium-cli -named createwallet "my=wallet"`,
