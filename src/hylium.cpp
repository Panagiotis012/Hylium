...
Options:
  -m, --multiprocess     Run multiprocess binaries hylium-node, hylium-gui.
  -M, --monolithic       Run monolithic binaries hyliumd, hylium-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'hylium-qt [ARGS]' or 'hylium-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'hyliumd [ARGS]' or 'hylium-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'hylium-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'hylium-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'hylium-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_hylium [ARGS]'.
  chainstate [ARGS] Run hylium kernel chainstate util, equivalent to running 'hylium-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_hylium [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_hylium-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "hylium-gui" : "hylium-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "hylium-node" : "hyliumd");
        } else if (cmd.command == "rpc") {
            args.emplace_back("hylium-cli");
            // Since "hylium rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("hylium-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("hylium-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_hylium");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("hylium-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_hylium");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_hylium-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("hylium-util");
        } else {
...
//! Execute the specified hyliumd, hylium-qt or other command line in `args`
...
//!             be a relative path to a hyliumd, hylium-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/hylium ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/hylium/hylium/pull/31375#discussion_r1861814807)
