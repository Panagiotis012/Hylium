...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking hylium code so they have to run
    ...
    // functions run, so client objects are fully disconnected before hylium
    // code in the destructors are run. This way if the hylium code tries to
...
void EventLoop::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In hylium core, running these callbacks in the right order is
