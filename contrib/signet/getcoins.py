#!/usr/bin/env python3
# Copyright (c) 2020-2022 The Hylium Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing hylium-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='hylium-cli', help='hylium-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Hylium address to which the faucet should send')
...
parser.add_argument('hylium_cli_args', nargs='*', help='Arguments to pass on to hylium-cli (default: -signet)')

args = parser.parse_args()

if args.hylium_cli_args == []:
    args.hylium_cli_args = ['-signet']


def hylium_cli(rpc_command_and_params):
    argv = [args.cmd] + args.hylium_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = hylium_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = hylium_cli(['getnewaddress', 'faucet', 'bech32'])
