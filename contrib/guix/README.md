# Bootstrappable Hylium Core Builds

This directory contains the files necessary to perform bootstrappable Hylium
Core builds.
...
To build Hylium Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Hylium Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/hylium-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/hylium-core/guix.sigs).
