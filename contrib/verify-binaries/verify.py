...
"""Script for verifying Hylium Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from hyliumcore.org and hylium.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/hylium-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://hyliumcore.org"
HOST2 = "https://hylium.org"
VERSIONPREFIX = "hylium-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Hylium community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"hylium_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by hyliumcore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the hylium release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes hylium.org lags behind hyliumcore.org.)')
    )
