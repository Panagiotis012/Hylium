...
be used in Hylium Core with the `-asmap` option.
...
to binary before being used with Hylium Core.
...
the `hylium-cli getnodeaddresses` command.
...
```
hylium-cli getnodeaddresses 0 > addrs.json
```
