...
Provides utilities for managing copyright headers of `The Hylium Core
...
Generates a hylium.conf file in `share/examples/` by parsing the output from `hyliumd --help`. This script is run during the
release process to include a hylium.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-hylium-conf.sh
```
